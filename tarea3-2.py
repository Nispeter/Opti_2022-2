...
file_name = SOLOMON_C201
max_vehicle  = file_name['v_num'] 
q = file_name['capacity']  
...
#SETEAR VARIABLES
x = pulp.LpVariable.dicts('x', ((i, j, k) for i in range(n_points) for j in range(n_points) for k in range (max_vehicle)), lowBound=0, upBound=1, cat='Binary')     #aristas seleccionadas para el recorrido
vehicles = pulp.LpVariable('vehicle', lowBound=0, upBound=max_vehicle, cat='Integer')                                               #cantidad de vehiculos a usar
#u = pulp.LpVariable.dicts('u', ((i,k) for i in range(n_points) for k in range(max_vehicle)), lowBound = 1, upBound = q, cat = 'Integer')                            #capacidad usada    
h = pulp.LpVariable.dicts('h', (i for i in range(n_points) ),lowBound = a[0], upBound = b[0], cat = 'Integer') 
#h = pulp.LpVariable.dicts('h', ((i,k) for i in range(n_points) for k in range (max_vehicle) ),lowBound = a[0], upBound = b[0], cat = 'Integer') 
...
for i in range(1,n_points): 
   problem += pulp.lpSum(x[i,j, k] for j in range(n_points) for k in range(max_vehicle) ) == 1 # Constraint (2&3)  
...
problem += pulp.lpSum([x[i, 0, k] for i in range(n_points) for k in range(max_vehicle)]) == vehicles # Constraint (4)
problem += pulp.lpSum([x[0, i, k] for i in range(n_points) for k in range(max_vehicle)]) == vehicles # Constraint (5)
    ...
    for j in range(n_points):
        for k in range(max_vehicle):
            if i != j and (i != 0 and j != 0):
                problem += h[i] + t[i] + dist[i][j]  <= h[j] + M * (1 - x[i,j,k]) #check de que se puede llegar al siguiente nodo por tiempo
                #problem += h[i] + t[i] + dist[i][j] - M * (1 - x[i,j,k]) <= h[j] #check de que se puede llegar al siguiente nodo por tiempo
for i in range(n_points):   
    problem += (h[i] >= a[i] and h[i] <= b[i])
    #problem += h[i] <= b[i]

for i in range(n_points):
    for k in range(max_vehicle):                #Constraint (6)
        problem += pulp.lpSum(x[j,i,k] for j in range(n_points) if i != j) - pulp.lpSum(x[i,j,k] for j in range(n_points) if i != j) == 0



#RESOLVER   
solver = pulp.getSolver('PULP_CBC_CMD', timeLimit=600) #120 seg.
...
#                print(f"k: {k}, i: {i}, j: {j},")
#print(f"v: {int(pulp.value(vehicles))}")

#RESTRICCIONES NO USADAS

#Grafo completo 
"""for j in range(1, n_points):
    for k in range (max_vehicle):
        problem += pulp.lpSum(x[j, i, k] for i in range(n_points) ) == 1 # Constraint (2)
for i in range(1, n_points):
    for k in range (max_vehicle):
        problem += pulp.lpSum(x[i, j, k] for j in range(n_points) ) == 1 # Contraint (3)     
"""
...
for k in range(max_vehicle):
    problem += pulp.lpSum(x[i, 0, k] for i in range(n_points)) == 1 # Constraint (4)
    problem += pulp.lpSum(x[0, i, k] for i in range(n_points)) == 1 # Constraint (5)"""
"""
#Capacidad minima comparando el siguiente nodo 
for i in range(n_points):
    for j in range(n_points):
        if j != 0 and i != 0: 
            for k in range(max_vehicle):
...
for i in range(n_points):
    for k in range(max_vehicle):
        if i!=0:
            problem += 1 <= u[i,k] <= q - d[i] # Constraint (7)"""
...
for i in range(n_points):
    for k in range(max_vehicle):
        problem +=  a[i] <=  h[i,k] <= b[i]    #Constraint
        #problem += a[i] <= h[i,k] + t[i] <= b[i]    #Constraint(8)
...
for i in range(n_points):
    for j in range(n_points):   
        for k in range(max_vehicle):
            if i != j :
                problem += h[i,k] + dist[i][j] + t[i] - M*(1-x[i,j,k]) <= h[j,k]      #Constraint (9)"""